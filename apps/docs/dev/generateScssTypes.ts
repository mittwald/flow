 ...
 *
 * - `configFile: false` — nothing must be auto-discovered; `vitest.config.ts` is
 *   not a Vite config and Next has none at all.
 * - The `@/…` alias mirrors `tsconfig.json`'s `paths` and `vitest.config.ts`, so
 *   a future `@use "@/…"` in a module resolves like everywhere else.
