...
[Table](/03-components/structure/table) dargestellt werden, wenn eine
...
kann der User über einen „Mehr anzeigen“-[Button](/03-components/actions/button)
...
- Die List wird häufig auf Übersichtsseiten eingesetzt, z. B. um alle Domains
  ...
  [Detailseiten](/02-foundations/05-ui-patterns/03-struktur) navigieren.
- Sie nimmt immer die volle Breite des Inhalts einer
  [LayoutCard](/03-components/structure/layout-card) ein.
- Wenn neben der List weiterer Inhalt auf der Seite vorhanden ist, sollte dieser
  ...
  das Nachladen über den „Mehr anzeigen“-[Button](/03-components/actions/button)
  vermieden.
...
[Table](/03-components/structure/table)-Component. Sind mehrere Ansichten
verfügbar, können User über den Ansichts-[Button](/03-components/actions/button)
...
Der Sortierungs-[Button](/03-components/actions/button) wird stets links neben
dem Filter-Button angezeigt. Die Standardsortierung sollte dem häufigsten
Anwendungsfall entsprechen (z. B. „Neueste zuerst“ bei einer Änderungshistorie).
Die aktive Sortierung wird im Sortierungs-Button angezeigt (siehe Abschnitt
Writing guidelines).

## Filter

Ein Klick auf den Filter-Button öffnet ein
[ContextMenu](/03-components/actions/context-menu), in dem Filter aktiviert oder
deaktiviert werden können.

- Unterstützt der Filter eine **Mehrfachauswahl**, werden die Optionen als
  [Checkbox](/03-components/form-controls/checkbox) dargestellt.
- Erfordert ein Filter **Einzelauswahl**, werden die Optionen als
  [RadioGroup](/03-components/form-controls/radio-group) angezeigt.
- Jeder **aktive Filter** wird durch eine [Badge](/03-components/status/badge)
  visualisiert, der per Klick einzeln entfernt werden kann. Sind mindestens zwei
  Filter aktiv, erscheint zusätzlich ein “Filter zurücksetzen”-Button.
...
anzeigen"-[Button](/03-components/actions/button) können bei Bedarf weitere
ListItems nachgeladen werden.
...
- **Avatar:** Der [Avatar](/03-components/content/avatar) steht am Anfang eines
  ListItems. Wird ein [Icon](/03-components/content/icon) verwendet, sollte es
  die Kategorie des dargestellten Elements widerspiegeln (z. B. ein Domain-Icon
  bei der Kategorie Domain). Falls der User für das Element ein
  [Image](/03-components/content/image) hochladen kann, wird dieses angezeigt.
  Ist kein Image vorhanden, sollten stattdessen Initialen erscheinen.
...
- **Aktionen:** Für Interaktionen mit dem ListItem stehen zwei Varianten zur
  ...
  [ContextMenu](/03-components/actions/context-menu) angeboten, oder sie
  erscheinen direkt als [Buttons](/03-components/actions/button) innerhalb des
  ListItems.
...
ausgerichtet. Der [Avatar](/03-components/content/avatar) wird hier größer und
in eckiger Form angezeigt. Die grundlegenden Funktionen sind vergleichbar mit
denen der Listenansicht, allerdings entfällt in diesem Modus der Top Content
sowie die Accordion-Funktion.

## Tabellenansicht

Wird eine List mit einer Tabellenansicht verwendet, sind die Guidelines der
[Table](/03-components/structure/table) zu beachten.

## IllustratedMessage

Die [IllustratedMessage](/03-components/content/illustrated-message) wird in
zwei Anwendungsfällen verwendet:

- **Keine Einträge vorhanden:** Die List wird durch eine IllustratedMessage
  ersetzt. Diese lädt den User über einen
  [Button](/03-components/actions/button) dazu ein, das erste Element zu
  erstellen.
...
Sortierungs-[Button](/03-components/actions/button) lediglich “Sortierung” an.
Wählt der User eine spezifische Sortierung aus, wird der Button-Text
entsprechend angepasst. Die Sortiermethode sollte so benannt werden, dass sofort
ersichtlich ist, nach welchem Kriterium und in welcher Reihenfolge sortiert
wird.
...
Diese sollten selbsterklärend sein. Bei mehrdeutigen Begriffen empfiehlt es
sich, zusätzlichen Kontext anzugeben.
...
Sortierung werden in einem Icon-[Button](/03-components/actions/button)
...
[ColumnLayouts](/03-components/structure/column-layout) auf kleinen Bildschirmen
ausgeblendet werden. Dabei dürfen jedoch nur Informationen ausgeblendet werden,
...
zusätzliche [Heading](/03-components/content/heading) erforderlich. In diesem
