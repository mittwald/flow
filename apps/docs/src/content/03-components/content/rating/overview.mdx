# Playground

Verwende `<Rating />`, um ein Rating darzustellen. Um die Barrierefreiheit zu gewährleisten, wird in der Rating Component automatisch ein entsprechendes ARIA-Label generiert.

<LiveCodeEditor />

---

# Sizes

Es stehen die Größen **Medium** oder **Small** zur Verfügung.

<LiveCodeEditor example="size" editorCollapsed />

---

# Kombiniere mit ...

## BigNumber

Das Rating kann mit einer [BigNumber](https://mittwald.github.io/flow/03-components/data-visualisation/big-number) kombiniert werden, um den Wert der BigNumber visuell besser einordnen zu können.

<LiveCodeEditor example="big-number" editorCollapsed />

## AccentBox

Zur visuellen Hervorhebung kann das Rating innerhalb einer [AccentBox](https://mittwald.github.io/flow/03-components/structure/accent-box) angezeigt werden.

<LiveCodeEditor example="accent-box" editorCollapsed />
