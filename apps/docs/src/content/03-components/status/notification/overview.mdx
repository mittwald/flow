...
Verwende `<Notification />`, um eine Notification darzustellen. Um eine Notification auszulösen und sie zu steuern, kann der [NotificationProvider](/03-components/status/notification-provider) genutzt werden. 
