...
Ein AlertBadge ist eine erweiterte Form der [Badge](/03-components/status/badge), der für kritische Hinweise und dringende Informationen eingesetzt wird. Es unterscheidet sich durch den Einsatz eines Icons und eine auffällige Farbgebung.

## Best practices

Achte bei der Verwendung einer AlertBadge darauf, dass...

- die Beschriftung kurz und prägnant ist.
- der richtige Status verwendet wird (Info, Success, Warning, Danger).
- ein AlertBadge nur eingesetzt wird, wenn der Hinweis kritisch oder besonders aufmerksamkeitsstark ist – andernfalls genügt ein [Badge](/03-components/status/badge).
