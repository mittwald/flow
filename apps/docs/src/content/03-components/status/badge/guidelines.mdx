...
- die Farben konsistent für bestimmte Bedeutungen genutzt werden (z. B. `<green />` = Aktiviert, `<blue />` = Info).
- mithilfe eines Scopes dem Badge eine übergeordnete Bezeichnung hinzugefügt wird.
- diese für Metadaten und Kategorien verwendet wird. Bei kritischere Informationen wird eher eine [AlertBadge](/03-components/status/alert-badge) in Kombination eines [Alert](/03-components/status/alert) verwendet.
