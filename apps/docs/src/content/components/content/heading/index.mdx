...
Verwende einen [Button](/components/actions/button), um eine Aktion direkt an
der Heading anzubieten – etwa eine Aktion, die nur diesen einen Abschnitt
betrifft. Der Button wird hinter dem Text der Heading angeordnet, bricht mit ihr
um und wird automatisch in der Size `s` dargestellt.

Aktionen, die den gesamten Abschnitt oder die Seite betreffen, gehören dagegen
in eine [ActionGroup](/components/actions/action-group) neben der Heading.
