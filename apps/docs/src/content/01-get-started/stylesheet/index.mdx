...
welche Regel gewinnt, entscheidet sich wie gewohnt über Spezifität und
Quell-Reihenfolge – gegenüber früheren Versionen ändert sich nichts.
...
- **Standard (`css` / `all.css`):** klassisches, spezifitätsbasiertes Verhalten,
  unverändert gegenüber früheren Versionen. Passt auch für Apps mit einem
  aggressiven globalen Reset (z. B. `* { all: initial }`), der gelayerte Styles
  sonst vollständig überschreiben würde.
