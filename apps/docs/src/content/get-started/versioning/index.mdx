...
Package-spezifischen Sonderregeln – ausgenommen ein Package, das als Ganzes Beta
ist (siehe [Component Lifecycle](#component-lifecycle)).
...
TypeScript-Typen, CSS-Klassennamen und Design Tokens fallen nicht unter Semantic
Versioning. Eine Änderung daran ist unwahrscheinlich – wir nehmen sie nicht
leichtfertig vor –, aber möglich: Sie kann in jedem Release erscheinen, auch in
einem Patch, ohne Major-Version und ohne Deprecation-Phase. Wenn du dich auf
einen davon stützt, lies „So schützt du dich“.

- **TypeScript-Typen.** Das gilt für die gesamte Type-Ebene: Auch das Entfernen
  oder Umbenennen eines exportierten Typs oder das Verengen eines Prop-Typs ist
  für sich genommen kein Breaking Change. Nennenswerte Type-Änderungen werden
  dennoch im Changelog erwähnt.
- **CSS-Klassennamen.** Ein internes Implementierungsdetail – weder ihre
  Existenz noch ihre Schreibweise ist zugesagt.
- **Namen und Werte von Design Tokens.** Ein Token kann umbenannt werden, einen
  anderen Wert bekommen oder verschwinden.
...
Weil die best-effort-Bereiche bewusst nicht garantiert sind, gibt es drei
Regeln, an die du dich als Entwickler halten solltest.

<Alert>
  <Heading>Drei Regeln für den stabilen Betrieb</Heading>
  <Content>
    Style nicht gegen interne CSS-Klassen, referenziere Design Tokens nicht
    ungeprüft über Releases hinweg, und behandle Flows TypeScript-Typen als
    best-effort. Alle drei Bereiche ändern sich selten – aber wenn, dann ohne
    Vorwarnung in einem beliebigen Release.
  </Content>
</Alert>
...
## Behandle Design Tokens als best-effort

Design Tokens sind für die Verwendung innerhalb von Flow gedacht. Greifst du in
eigenem Code auf eine Token-CSS-Variable zu, prüfe sie nach einem Update: Name
und Wert können sich auch in einem Patch ändern, und ein nicht mehr
existierender Token fällt still auf den Browser-Default zurück statt einen
Fehler zu werfen.
...
Auch ein ganzes Package kann Beta sein. Das gilt derzeit für
`@mittwald/flow-remote-vue-components`, die Vue-Anbindung für
mStudio-Extensions: Es teilt die gemeinsame Version, ist aber von der
Breaking-Change-Zusage ausgenommen.
