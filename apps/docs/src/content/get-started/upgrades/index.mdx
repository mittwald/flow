...
`upgrade patch` bleibt auf deiner Minor, `upgrade major` sucht auch über die
Major-Grenze hinaus – ist dort nichts veröffentlicht, landet es in deiner
aktuellen Major und sagt das –, und eine exakte Version oder ein Dist-Tag
(`next`) gehen genau dorthin.
...
migration in that range. Three runs are often fewer than three options, in two
different ways:

- A keyword **fails**. From a prerelease, or from the newest version on a line,
  `patch` and `minor` have no release to point at and exit non-zero with no JSON
  and a message saying so.
- A keyword **succeeds and lands where a narrower one already does**. `major` is
  unbounded rather than "the next major": with no newer major published it
  resolves inside the current one, which is the same version `minor` reaches.
  `range.stayedWithin` in the JSON says so — `"major"` or `"minor"`. Without
  reading it, the two reports differ in nothing but the revision that produced
  them.

Neither is an error to work around.

So show me what actually resolved, side by side: target version, how many
migrations carry a codemod, how many need a hand, and how many are new rather
than catch-up — `catchUp: false` in the JSON; the rest shipped before the
version this project is already on. Collapse targets that are the same version
into one row. If fewer than three distinct targets remain, say which and why,
and put a dist-tag (`next`) or an exact version (`1.4.0`) on the table as an
equal option rather than a fallback — those always resolve.
...
- It bumps every package published from the Flow monorepo, not only the
  ...
  that is exactly the lines it printed, the Flow dependencies declared in that
  one `package.json`. Flow packages that arrive only as transitive dependencies
  move with the install; they are in no manifest and in no report, so do not go
  mining the lockfile to pad the list.
- Only if more than one package declares Flow: run it once per package, from
  ...
  Those commits are the migration's commits, not a workaround for the guard —
  the closing round at the end only covers what comes after them.
...
- If the install fails for any other reason, do not wait and retry — that only
  ...
  are not.
- If the install **succeeds**, read the line about files it also changed. The
  install belongs to the package manager, not to this tool, and it may rewrite
  more than the manifest and the lockfile; anything else it touched is named
  there. On pnpm 11 that is usually `pnpm-workspace.yaml`: an explicit bump makes
  pnpm add a `minimumReleaseAgeExclude` entry, so its own `minimumReleaseAge`
  does not refuse a version published minutes ago. pnpm 11.5 appends a **second**
  entry for a package that already has one, and pnpm's `verifyDepsBeforeRun` then
  matches the first and ignores the second — after which every script in that
  package dies with `ERR_PNPM_MINIMUM_RELEASE_AGE_VIOLATION` before running
  anything. Merge the duplicate entries into one line. This is not an install
  failure, retrying does not help, and the symptom only surfaces later, in the
  checks below.
...
as a result. Read the `upgrade` report for what it admits. Each codemod prints
its counts — `N file(s) changed, M unchanged`, plus `K declined` and `E empty`
when those are not zero — and each number says something different:

- **`M unchanged` is how much of the tree the codemod actually read.** `0 file(s)
  changed, 0 unchanged` and `0 file(s) changed, 240 unchanged` are opposite
  results: the first looked at nothing, the second looked at everything and
  found nothing. Never read a bare "0 changed" as either.
- **`declined` is a file the transform read and bailed on** — a shape it could
  not decide from the source. That is exactly where the old pattern hides, so
  weight those entries first in the pass below.
- **`empty` is a 0-byte source file.** It has nothing to do with the migration.

Three lines are verdicts rather than counts, and all three mean that migration
got no usable run: `failed to run — …` (the codemod threw), `no files under
<path> were processed` (nothing matched, or `--path` is wrong) and `declined all
N file(s) it looked at` (it bailed on every file it saw). Tell me if any of them
appears — the by-hand pass below is then the only thing covering that entry.
...
`action` has three values, not two. `codemod` means a transform exists and has
already run; `manual` means there is an edit to make by hand; `none` means the
behaviour changed with nothing to rewrite — read `apply` and judge whether the
new behaviour is a problem here, then say so. The version an entry shipped in is
`since`.
...
Finally, do not leave the migration sitting uncommitted. In a monorepo you
already committed once per package above; those commits stand, and what is left
is everything that came after them — the by-hand edits, the fixes to what the
upgrade broke, the decisions we just settled. Walk me through the diff at the
level of what changed and why — not file by file — then propose how to commit
the remainder, with the messages written out. If the tree is already clean, say
that instead and list the commits you made. Commit once I say so. If I would
rather review first, leave the tree as it is and say so in one line; do not
commit without an answer, and do not push in any case.
