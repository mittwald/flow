...
Two of the three can also resolve to the same version. A keyword bounds the
target from above and nothing more, so `major` finds the highest published
stable release whether or not that crosses a major boundary — with no `2.x`
published it lands inside `1.x`, exits 0, and its JSON differs from `minor`'s in
`range.target` alone. `range.note` says when that happened. Compare the three
targets rather than assuming three choices.

So show me what actually resolved, side by side: target version, how many
migrations carry a codemod, how many need a hand. Collapse the keywords that
resolved to the same version into one row and say which they were. If fewer than
three distinct targets remain, say which and why, and put a dist-tag (`next`) or
an exact version (`1.4.0`) on the table as an equal option rather than a
fallback — those always resolve.

Each entry carries `catchUp`: true means it shipped at or before the version
this project is on, so the range you are comparing is not the same as the work
that is new. Count both.
...
- If the install fails for any other reason, do not wait and retry — that only
  ...
  are not.
- A successful install is not the end of it. pnpm 11 refuses a dependency
  younger than its `minimumReleaseAge` and writes a `minimumReleaseAgeExclude`
  entry into `pnpm-workspace.yaml` to let the explicit bump through — so the
  install passes and every later `pnpm run` in that package fails with
  `ERR_PNPM_MINIMUM_RELEASE_AGE_VIOLATION` before running anything, if the
  exclusion did not take. Check `git status` for a changed `pnpm-workspace.yaml`
  after the install and tell me what went in; some pnpm versions append a second
  entry for a package that already has one instead of merging it, and pnpm's own
  pre-run check then reads only the first. Merging the two by hand is the fix.
  Nothing in the tool's report mentions the file.
...
`action` has a third value besides `codemod` and `manual`: `none`. Behaviour
changed and no code change is required. Check those against the code like any
other entry — what they need is a decision that nothing broke, not an edit — and
say so rather than hunting for something to rewrite.

Two caveats on that report. "No files were processed. Is the path right?" is
also what a codemod that crashed before touching a file looks like — the
counters are identical, and the summary line guesses at the path. Check stderr
for a stack trace before you believe the path is wrong, and tell me if a codemod
died: its migration then had no run at all, and everything below is the only
thing that covers it.

And read the per-codemod line whole. `0 file(s) changed, 3 unchanged` means the
codemod read three files and left them alone; `0 file(s) changed, 0 unchanged`
means it read nothing that parsed. The `unchanged` count is how you tell a run
that covered the tree from one that covered a corner of it — if it is far below
the number of source files you expect, the path is wrong, not the codemod.
...
Finally, do not leave the migration sitting uncommitted. Walk me through the
diff at the level of what changed and why — not file by file — then propose how
to commit it, with the messages written out. Commit once I say so. If I would
...
If running `upgrade` once per package already made you commit after each one,
those are the commits — do not redo them. What is left here is everything since:
the edits you made by hand, the fixes to what the checks found, and whatever the
closing decisions changed. Walk me through the diff of the whole migration, but
propose commits only for what is still uncommitted.
