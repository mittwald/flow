...
| Modal                           | Ersetzt normale Modals                                         | <LiveCodeEditor example="modal-error" editorDisabled zoom=".6" />                      | <LiveCodeEditor example="modal-loading" editorDisabled  /> (Loading View der Flow Komponente) |
...
Für Texte gibt es die `WithBoundaries.Text` Komponente, bzw die
`WithBoundaries.String` Komponente, je nachdem ob am Ende eine `Text` Komponente
...
Zusätzlich gibt es Komponenten, die direkt auf ein Model angewendet werden
können und eine eingebaute Boundary besitzen.
...
Für Fields kann die `WithBoundaries.FieldFragment` Komponente verwendet werden.
