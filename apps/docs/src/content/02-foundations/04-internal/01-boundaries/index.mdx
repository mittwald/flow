...
umschließt die gesamte LayoutCard eine `LayoutCardWithBoundaries`. Über
`sectionsCount` wird die Loading View passend dimensioniert.

### Listenseite

Eine Listenseite enthält meist eine einzelne Section (`sectionsCount: 1`). Der
Count der Liste wird zuerst geladen, damit im Fehlerfall die gesamte Liste in
die ErrorView wechselt.
...
`sectionsCount` entspricht der erwarteten Anzahl Sections, damit das Layout
stabil bleibt. Die **Primär**-Ressource des Routen-Parameters wird zuerst
geladen, damit im Fehlerfall die ganze Seite in die ErrorView wechselt. Die
einzelnen `…Section`-Components lesen nur diese Ressource und bekommen daher
**keine eigene Boundary** (siehe
[Error-Scope](#error-scope-kritikalität-der-daten)).
