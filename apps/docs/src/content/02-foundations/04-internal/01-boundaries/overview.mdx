...
Der `sectionsCount` sollte der erwarteten Anzahl an Sections entsprechen. So bleibt das Layout während des Ladevorgangs stabil. Das Hauptelement der Seite wir zuerst geladen, damit im Fehlerfall für die gesamte Seite eine ErrorView angezeigt werden kann.
...
Für Modals sollte immer in ein `WithBoundaries.Modal` verwendet werden. Dies verhindert auch das Nachladen von Daten innerhalb des Modals, bevor das Modal geöffnet wurde.

### Inhalte von Forms in Modals

Eine teilweise geladene oder fehlerhafte Form untergräbt das Vertrauen der User
und kann zu unvollständigen oder inkonsistenten Daten führen. Sobald
innerhalb der Form ein Fehler aufftritt, fällt das automatisch das ganze Modal
in eine ErrorView, auch wenn es darin noch Boundaries gibt.
...
Kleine Elemente wie Texte, Links, Badges, Actions oder ContextMenus werden
oft dynamisch geladen, da sie das Nutzungserlebnis der Seite jediglich ergänzen. Diese sollten daher seperat geschützt werden, um
nicht die gesamte Seite zu blockieren. Diese Elemente besitzen oft keine eigene ErrorView und erscheinen im Fehlerfall nicht oder verbleiben in ihrem Ladezustand.
...
Alerts und Badges ergänzen eine Seite oft nur um kleine Hinweise oder Statusinformationen. Sie werden in ein `WithBoundaries.Fragment` verpackt. So bleiben sie geschützt, ohne eine eigene Loading View auszulösen.
...
### Field (WithBoundaries.FieldFragement)
