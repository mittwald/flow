...
Ordne mit `<ColumnLayout />` Inhalte in Spalten an. Anzahl und Größenverhältnis
der Spalten lassen sich je Breakpoint festlegen.
...
Achte bei der Verwendung eines ColumnLayouts darauf, dass ...

- nicht zu viele Spalten verwendet werden – in der Regel reichen maximal 4.
- die Spaltenbreiten bewusst gewählt werden: einheitliche Breiten wirken
  harmonisch, unterschiedliche lenken den Fokus auf breitere Inhalte.
- es bei Bedarf mit einer [Section](/04-components/structure/section/overview)
  kombiniert wird, um übersichtliche Layouts zu erstellen.
- mehrere ColumnLayouts ineinander verschachtelt werden, um Inhalte optisch zu
  gruppieren.
