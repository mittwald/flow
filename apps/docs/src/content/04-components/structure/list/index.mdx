...
Erzeuge mit `typedList<T>` eine für deinen Datentyp typisierte List. Über
Children wie `<List.Item />`, `<List.Search />`, `<List.Filter />` und
`<List.Sorting />` legst du Darstellung und Funktionsumfang fest.
...
- ein spezifisches ListItem über Filter, Sortierung oder Suche zu finden.
- über ein ListItem zur [Detailseite](/03-patterns/01-patterns/detail-page)
  eines Eintrags zu navigieren.
...
- eine passende Ansicht angeboten wird. Eine Rasteransicht ist z. B. sinnvoll,
  ...
  mehrere Anwendungsfälle, sollte der User zwischen Ansichten wechseln können.
- eine Standardsortierung gewählt wird, die dem häufigsten Anwendungsfall
  entspricht (z. B. „Neueste zuerst“ bei einer Änderungshistorie), und nur
  Sortieroptionen mit echtem Mehrwert angeboten werden.
...
- weiterer Seiteninhalt oberhalb der List platziert wird, damit das Nachladen
  über den „Mehr anzeigen“-Button keine Layout-Verschiebungen verursacht. Die
  List nimmt dabei die volle Breite des Contents einer
  [LayoutCard](/04-components/structure/layout-card/overview) ein.
- sie zugänglich beschrieben ist: Ist die List der einzige Hauptinhalt einer
  Seite, erhält sie ein `aria-label`. Hat sie eine eigene
  [Heading](/04-components/content/heading/overview), wird diese über
  `aria-labelledby` zugeordnet.
...
[Table](/04-components/structure/table/overview) darzustellen – dabei gelten die
...
- **Avatar:** Der [Avatar](/04-components/content/avatar/overview) steht am
  Anfang. Ein [Icon](/04-components/content/icon/overview) spiegelt die
  Kategorie wider (z. B. ein Domain-Icon); ein hochgeladenes
  [Image](/04-components/content/image/overview) wird angezeigt, andernfalls
  erscheinen Initialen.
...
- **Aktionen:** Interaktionen erfolgen über ein
  [ContextMenu](/04-components/actions/context-menu/overview) oder direkt als
  [Buttons](/04-components/actions/button/overview) im ListItem.
...
werden automatisch am Anfang der Zeile angeordnet. Ihre Funktionalität wird
nicht von der List gesteuert und muss individuell implementiert werden. Achte
darauf, dass die gesamte Zeile zur Auswahl genutzt werden kann – nutze dafür
`onAction` der List.
...
`m` und `l` mitgegeben werden, um Seitenverhältnis und Umbruchverhalten von
Header und Content zu steuern.
...
[Buttons](/04-components/actions/button/overview) im ListItem zu platzieren.
...
- Standardmäßig erlaubt ein Filter die **Mehrfachauswahl**, damit User nach
  ...
  [Checkbox](/04-components/form-controls/checkbox/overview). Bei
  **Einzelauswahl** werden sie als
  [RadioGroup](/04-components/form-controls/radio-group/overview) dargestellt.
- Jeder **aktive Filter** wird durch eine
  [Badge](/04-components/status/badge/overview) visualisiert, die per Klick
  entfernt werden kann. Sind mindestens zwei Filter aktiv, erscheint zusätzlich
  ein „Filter zurücksetzen“-Button.
...
[Modal](/04-components/overlays/modal/overview), fokussiere das Suchfeld über
...
`<TableCell />` in der Tabellenansicht – lässt sich diese Ansicht anpassen.
Gestalte sie mit [Skeleton](/04-components/content/skeleton/overview) und
...
den User über einen [Button](/04-components/actions/button/overview) einlädt,
das erste Element zu erstellen. Liefert eine Suche oder ein Filter kein
Ergebnis, zeigt `emptySearchResultView` einen entsprechenden Hinweis. Ist das
jeweilige Property nicht gesetzt, verwendet die List eine vordefinierte Ansicht.
...
kann über [ColumnLayouts](/04-components/structure/column-layout/overview) auf
kleinen Bildschirmen ausgeblendet werden – dabei dürfen nur Informationen
entfallen, die der User nicht benötigt, um das ListItem zu verstehen.
...
[ActionGroup](/04-components/actions/action-group/overview) mit Aktionen
anzuzeigen, die sich direkt auf die Liste beziehen.
