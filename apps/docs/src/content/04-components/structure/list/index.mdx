...
- **Biete eine passende Ansicht an.** Eine Rasteransicht eignet sich, wenn das
  Bild eines ListItems den Userflow bestimmt; bei mehreren Anwendungsfällen kann
  der User zwischen Ansichten wechseln.
- **Wähle eine Standardsortierung nach dem häufigsten Anwendungsfall.** Ein
  Beispiel ist „Neueste zuerst“ bei einer Änderungshistorie; biete nur
  Sortieroptionen mit echtem Mehrwert.
...
- **Platziere weiteren Seiteninhalt oberhalb der List.** So verursacht das
  Nachladen über den „Mehr anzeigen“-Button keine Layout-Verschiebungen; die
  List nimmt die volle Breite des Contents einer
  [LayoutCard](/04-components/structure/layout-card/overview) ein.
- **Beschreibe die List zugänglich.** Ist sie der einzige Hauptinhalt einer
