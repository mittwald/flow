...
[Table](/04-components/structure/table) dargestellt werden, wenn eine
...
kann der User über einen „Mehr anzeigen“-[Button](/04-components/actions/button)
...
- Die List wird häufig auf Übersichtsseiten eingesetzt, z. B. um alle Domains
  ...
  [Detailseiten](/03-patterns/struktur) navigieren.
...
- Wenn neben der List weiterer Inhalt auf der Seite vorhanden ist, sollte dieser
  ...
  das Nachladen über den „Mehr anzeigen“-[Button](/04-components/actions/button)
  vermieden.
...
[Table](/04-components/structure/table)-Component. Sind mehrere Ansichten
verfügbar, können User über den Ansichts-[Button](/04-components/actions/button)
...
Der Sortierungs-[Button](/04-components/actions/button) wird stets links neben
...
- Unterstützt der Filter eine **Mehrfachauswahl**, werden die Optionen als
  [Checkbox](/04-components/form-controls/checkbox) dargestellt.
- Erfordert ein Filter **Einzelauswahl**, werden die Optionen als
  [RadioGroup](/04-components/form-controls/radio-group) angezeigt.
...
anzeigen"-[Button](/04-components/actions/button) können bei Bedarf weitere
...
- **Avatar:** Der [Avatar](/04-components/content/avatar) steht am Anfang eines
  ListItems. Wird ein [Icon](/04-components/content/icon) verwendet, sollte es
  ...
  [Image](/04-components/content/image) hochladen kann, wird dieses angezeigt.
...
- **Aktionen:** Für Interaktionen mit dem ListItem stehen zwei Varianten zur
  ...
  [ContextMenu](/04-components/actions/context-menu) angeboten, oder sie
  erscheinen direkt als [Buttons](/04-components/actions/button) innerhalb des
  ListItems.
...
ausgerichtet. Der [Avatar](/04-components/content/avatar) wird hier größer und
...
[Table](/04-components/structure/table) zu beachten.

## IllustratedMessage

Die [IllustratedMessage](/04-components/content/illustrated-message) wird in
...
- **Keine Einträge vorhanden:** Die List wird durch eine IllustratedMessage
  ...
  [Button](/04-components/actions/button) dazu ein, das erste Element zu
  erstellen.
...
Sortierungs-[Button](/04-components/actions/button) lediglich “Sortierung” an.
...
Aktive Filter werden als [Badges](/04-components/status/badge) dargestellt.
...
Sortierung werden in einem Icon-[Button](/04-components/actions/button)
...
[ColumnLayouts](/04-components/structure/column-layout) auf kleinen Bildschirmen
