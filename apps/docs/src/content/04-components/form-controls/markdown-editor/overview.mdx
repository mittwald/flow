...
Ergänze die Toolbar des Markdown Editors, indem du zusätzliche Actions direkt
als Children von `<MarkdownEditor />` deklarierst. So lassen sich einfache
Buttons genauso integrieren wie komplexere Trigger-Komponenten wie
`<ContextMenuTrigger />` oder `<ModalTrigger />`.
