...
Füge in der `<ComboBox />` ein `<Label />` und die auswählbaren
`<Option />`-Elemente ein. Bei der Eingabe filtert die ComboBox die Liste
dynamisch; eine freie Eingabe außerhalb der Optionen ist standardmäßig nicht
vorgesehen.
...
Benutze [Align](/04-components/structure/align/overview), um einen
