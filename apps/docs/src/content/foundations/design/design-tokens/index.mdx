...
**color--mittwald-blue--800**

Der Token `color--mittwald-blue--800` definiert eine spezifische
[Color](/foundations/design/colors). `color` gibt den **Context** an. Der Name
der Farbe `mittwald-blue` ist die **Common Unit**. Die Zahl `800` ist die
**Clarification**, die die Abstufung der Farbe definiert und auf den genauen
Farbwert `#2D23E8` im Designsystem verweist.
...
in diesem Fall einen [Alert](/components/status/alert). `alert` liefert den
...
| Token-Name                | Wert    |
| ------------------------- | ------- |
| color--mittwald-blue--800 | #2D23E8 |
| size-px--s                | 8px     |
...
Zusammenhang (Primary, Success, Danger, ...) sie genutzt werden soll. Sie
...
| Token-Name          | Wert                      |
| ------------------- | ------------------------- |
| primary--color--800 | color--mittwald-blue--800 |
...
Diese Tokens sind speziell auf einzelne Components zugeschnitten. Sie vermitteln
einzigartige Designentscheidungen, die ausschließlich für eine bestimmte
Component relevant sind. Sie enthalten den höchsten Detailgrad der
