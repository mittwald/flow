...
| `packages/codemods`                     | private                                  | jscodeshift migrations for consumers. Transforms are fetched by URL — see its `AGENTS.md`. |
...
| A codemod a consumer runs from a raw GitHub URL dies with **`Cannot find module './<sibling>'`**, while the same transform works in this repo                                                                   | jscodeshift downloads a `-t <url>` transform into the OS temp dir and requires it from there, so its relative imports resolve against that temp dir, where no sibling exists                                                                                                                      | Every file in `packages/codemods/src/transforms` must be self-contained. A transform that composes others belongs in `src/composites`; `pnpm nx build codemods` inlines it into a standalone, committed bundle. `src/tests/standalone.test.ts` guards this                                                                                                                                                       |
...
| Consumer codemods                           | [packages/codemods/AGENTS.md](packages/codemods/AGENTS.md)                              |
