...
| `pnpm format` rewrites **dozens of files you never touched** (`.md`, `.mdx`, `.json`, `.yml`, `lerna.json`, `AGENTS.md`, …)                           | Only `.ts`/`.tsx` are Prettier-gated in CI (via `eslint-plugin-prettier`), so the other file types drift from whatever Prettier version last formatted them                                                                                                                                       | Format only your own changes: `pnpm prettier --write $(git diff --name-only) <new files>`. Never commit the unrelated churn — and don't try to strip it afterwards with a bulk `git checkout --`, that is how you lose your own edits                                                    |
