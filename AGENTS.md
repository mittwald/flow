...
| A `:global(.flow--…)` selector silently matches nothing — a declaration is lost, but there is no lint error, no type error, no console warning, and the docs site and Storybook keep rendering                                                                               | The class-name generator (`packages/components/dev/vite/cssModuleClassNameGenerator.ts`) **drops the suffix when it equals the component name**, so `ColumnLayout`'s `.columnLayout` compiles to `flow--column-layout`, not `flow--column-layout--column-layout`. Nothing in the repo checks that a hand-written `:global(.flow--…)` name resolves                                                                                                                                      | Read the name out of the built `packages/components/dist/css/all.css` instead of deriving it by hand (`pnpm nx build components`, then grep). The visual suite is the only gate that catches this class of bug — it is what caught it in #3091                                                                                                                                                                   |
