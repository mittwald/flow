...
| An app/dev server errors with **"X is not exported from `@mittwald/flow-…`"** after adding/changing an `@flr-generate` component | You ran the app's own script (`next dev`, bare `vite`) directly, so the `^build` chain that regenerates `view.ts` / `src/auto-generated/**` in the dependency packages never ran | Start apps through their **nx target** — `pnpm nx run remote-dom-demo:dev` / `pnpm nx dev <app>` — whose `dependsOn: ["^build", …]` builds and regenerates every workspace dependency first                             |
