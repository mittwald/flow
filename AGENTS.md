...
| The serial visual run dies mid-suite with **"Browser connection was closed while running tests. Was the page closed unexpectedly?"** (`[birpc] rpc is closed, cannot call "createTesters"`), at about the same file count every time, while the browser's memory climbed by gigabytes | Playwright's WebKit never releases a removed same-origin iframe's document (Chromium and Firefox do). Vitest's default `isolate: true` runs every test file in a fresh iframe and removes the previous one, so each finished file leaves its whole realm behind — ~200 MB in the visual suite — until the page dies; the unsharded `update-screenshots` run made it to file 105 of 168 (#3119)                                                                                          | Keep `isolate: false` on a serial browser project that plays many files through one page (the visual project has it): one tester iframe for the whole run, no realm churn. Neither forcing GC (`page.requestGC()` frees nothing — the documents stay reachable) nor navigating the iframe to `about:blank` before removing it helps; sharding only bounds the page's lifetime                                    |
