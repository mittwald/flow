...
| A vite/vitest config you added floods every build and test run with **"Your Vite config uses features that are unsupported by `configLoader: 'native'`"**                                                                                                                    | Vite is moving to a native (Node) config loader, which resolves neither extensionless relative imports nor a directory's `package.json` `main`, and needs an import attribute on JSON                                                                                                                                                                                                                                                                                                   | Spell the imports out: `./vite.config.ts` (not `./vite.config`), `../core/src/index.ts` (not `../core`), `import pkg from "./package.json" with { type: "json" }`. Add `"allowImportingTsExtensions": true` to the package's `tsconfig.vite.json` so `test:compile` accepts the extensions. The rule reaches everything the config imports transitively, `packages/core/src` and `dev/vite/*` included                                                                                                                                                                                                |
...
| One environment of a visual scenario fails while the other passes, and the **`update-screenshots`** label reports _"No Screenshot Updates Required"_                                                                                                                         | `resolveScreenshotPath` keys the reference on the screenshot description, not the test name, so `(Local)` and `(Remote)` share **one** baseline file. `--update` lets each write it in turn and the last one wins — where the two environments render differently, the run ends with the loser's image and `git status` is clean                                                                                                                                                        | Treat it as a real divergence, not a baseline problem: the shared reference is the suite's way of asserting that both environments render the same. Capture each side on its own — delete the reference, then run the scenario filtered to one environment with `--update`, once per side — and compare the two files, then fix whichever side is wrong. A props context keyed on an authoring name (`ModalTrigger`) covers `Local` only — the host materialises what the _view_ emits (`DialogTrigger`)                                                                                              |
...
| The **`update-screenshots`** label pushed its commit and the PR now looks settled — but `main`, `visual`, `unit` and `browser` are **missing from the check list entirely**                                                                                                  | The label pushes as `github-actions[bot]` with the default `GITHUB_TOKEN`, which trips the org's workflow approval gate: every run created for the new head comes back `status=completed, conclusion=action_required` — created, never executed. `gh pr checks` reports nothing pending, so "wait until nothing is pending" reads as all-green on a commit that has no test run at all                                                                                                  | Check by SHA, not by absence of pending: `gh api "repos/mittwald/flow/actions/runs?head_sha=<sha>" --jq '.workflow_runs[] \| "\(.name) \(.status)/\(.conclusion)"'`. `approve-workflow-run` returns **404** — those runs are already `completed`, not awaiting approval (`pending_deployments` is empty). `gh run rerun <id>` from a write-access account is what executes them                                                                                                                                                                                                                       |
| A function prop works locally but its result is garbage remotely — a formatter renders `[object Promise]`, a predicate is always truthy                                                                                                                                      | The prop is a **remote property**, and `FlowThreadSerialization.isSerializableByBase` accepts any function, so `@quilted/threads` sends it as a proxy. Calling a proxy is a round trip, so the host gets a **Promise**, never the value. Only `on*` events are safe by construction — they are fire-and-forget                                                                                                                                                                          | Either type the prop's return as `Promise<T> \| T` and have the host `await` it (`ChartTooltip`'s formatters do), or drop it from the remote surface with `@flr-ignore-props` (`XAxis`/`YAxis` `tickFormatter` do) and regenerate. `checkSerializableProps` now fails generation on new ones; the pre-existing set is listed in `acknowledgedValueReturningProps`                                                                                                                                                                                                                                     |
...
| `stylelint` fails with **`flow/no-unknown-global-flow-class`** — a `:global(.flow--…)` name resolves to nothing                                                                                                                                                              | The class-name generator (`packages/components/dev/vite/cssModuleClassNameGenerator.ts`) **drops the suffix when it equals the component name**, so `ColumnLayout`'s `.columnLayout` compiles to `flow--column-layout`, not `flow--column-layout--column-layout`. The names cannot be derived by hand, and a component that moves leaves every reference to it dead                                                                                                                     | Copy the name from the component's committed `*.module.d.scss.ts` — that is what the rule checks against — or take the rule's "Did you mean" suggestion. Just added the class? Regenerate the stubs first: `pnpm nx build:scss-types components`. Before the rule (#3091) this class of bug failed completely silently: no build error, no console warning, and the docs site and Storybook kept rendering                                                                                                                                                                                            |
