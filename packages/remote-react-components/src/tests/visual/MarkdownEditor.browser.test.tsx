...
test.each(testEnvironments)(
  ...
  async ({ container, render, components: { MarkdownEditor, Label } }) => {
    await render(
    ...
    await expect(container).toMatchScreenshot("MarkdownEditor - default");

    await userEvent.type(input, "A\n**B**\nC\nD");

    await expect(container).toMatchScreenshot("MarkdownEditor - text entered");

    await preview.click();

    await expect(container).toMatchScreenshot("MarkdownEditor - preview");
  },
);
