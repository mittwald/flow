...
test.each(testEnvironments)(
  ...
  async ({
    ...
    await expect(container).toMatchScreenshot("SearchField states");
  },
);

test.each(testEnvironments)(
  "SearchField interaction (%s)",
  async ({ container, render, components: { SearchField, Label } }) => {
    await render(
      <SearchField>
        <Label>Label</Label>
      </SearchField>,
    ...
    await expect(container).toMatchScreenshot(
      "SearchField interaction - default",
    );

    await userEvent.type(input, "asdf");

    await expect(container).toMatchScreenshot(
      "SearchField interaction - text entered",
    );
  },
);
