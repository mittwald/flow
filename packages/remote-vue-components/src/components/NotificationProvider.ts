import { callHandlers } from "@/lib/handlers";
import { readBooleanProp } from "@/lib/propKeys";
import {
  cloneVNode,
  defineComponent,
  Fragment,
...
export class NotificationController {
  ...
  public add(notification: VNode): number {
    ...
    if (readBooleanProp(notification.props, "autoClose")) {
  ...
  public resumeAutoClose(entry: NotificationEntry): void {
    if (
      !readBooleanProp(entry.element.props, "autoClose") ||
      entry.autoCloseTimeout
    ) {
      return;
    }
...
export const NotificationProvider = defineComponent({
  ...
  setup(_props, { slots }) {
    ...
    /*
     * Replaced, not merged, like React's `cloneElement`: `cloneVNode` would
     * chain the app's `onClose` with this one, which calls it as well.
     */
    const renderEntry = (entry: NotificationEntry): VNode => {
      const notification = cloneVNode(entry.element, { key: entry.id });
      notification.props = {
        ...notification.props,
        ...
        onClose: () => {
          ...
          callHandlers(entry.element.props?.onClose);
        },
      };
      return notification;
    };
    ...
    return () => [
      h(Fragment, controller.notifications.map(renderEntry)),
      slots.default?.(),
    ];
  },
});

composition(NotificationProvider);
