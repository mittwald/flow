...
/**
 * What an icon is, typed so JSX accepts it.
 *
 * Not `Component`: that union includes a plain options object, which has no
 * call signature — and a JSX element type needs one, so `<IconSearch />` is a
 * type error for anyone writing JSX. The value is the same either way; only the
 * declared type is the constructible half of the union.
 */
...
export const svgIcon = (name: IconName, root: SvgNode): FlowIconComponent =>
