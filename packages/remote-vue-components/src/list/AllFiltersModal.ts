...
interface ModalDateRangeFilter {
  readonly name?: string;
  readonly property: unknown;
...
const renderDateRangeAccordion = (
  filter: ModalDateRangeFilter,
  expanded: boolean,
  ...
  return accordion(`filter-${name}`, name, expanded, () =>
    h(Flex, { direction: "column", gap: "m" }, () => [
      h(RangeCalendar, {
        ...filter.dateRangeOptions,
        value,
        onChange: (range: unknown) => filter.setValue(range),
      } as never),
      value
        ? h(
            Button,
      ...
      const accordions: VNodeChild[] = [
        ...
        ...filters.map((filter) =>
          isDateRangeFilter(filter)
            ? renderDateRangeAccordion(
                filter as unknown as ModalDateRangeFilter,
                expanded,
                format,
              )
