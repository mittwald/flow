...
import type { ListDateRange } from "@mittwald/flow-components-base";
import { defineComponent, h, type VNodeChild } from "vue";
import { formatDateRange, isDateRangeFilter } from "./dateRangeFilter";
...
interface ChipDateRangeFilter {
  readonly name?: string;
  getValue(): ListDateRange | null;
  clear(): void;
}
  ...
  setup(props) {
    ...
    return () => {
      ...
      const chips = filters.flatMap((filter) => {
        /* One chip for the whole range, and closing it clears the range. */
        if (isDateRangeFilter(filter)) {
          const range = (filter as unknown as ChipDateRangeFilter).getValue();
          ...
          return range
            ? [
                h(
                  Badge,
                  {
                    key: filter.storageKey,
                    isDisabled: props.isDisabled,
                    onClose: () =>
                      (filter as unknown as ChipDateRangeFilter).clear(),
                  },
                  () => h(Text, null, () => formatDateRange(range)),
                ),
              ]
            : [];
        }
