how the class names are structured.
...
smaller ones. Take the Note example. The Note is composed out of an icon, a
heading and some optional content. These contained components must have their
basic class name (`flow--icon`), to get their basic styling and the
specialization class name (`flow--note--status-icon`) to get the Note specific
styles.
