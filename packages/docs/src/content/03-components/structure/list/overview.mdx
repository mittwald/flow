...
Verwende `ActionGroup` innerhalb der List, um Aktionen für die Liste zu
definieren.
...
Die List kann als Liste oder als Tabelle dargestellt werden. Für die Listen
Darstellung benutzt du `<List.Item />` und für die Tabellen Darstellung
`<List.Table />`.
