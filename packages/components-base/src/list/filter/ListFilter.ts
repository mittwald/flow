...
> {
  ...
  private valuesFromTableState?: {
    facets: Map<unknown, number>;
    values: ListFilterValue<TRendered>[];
  };
  ...
  /*
   * Rebuilt only when TanStack hands out a new map: it memoizes the faceted
   * values and replaces the map when the data changes. Without this, every
   * row the filter function checks re-hashes every value — `getArrayValue()`
   * resolves each stored id through `values`.
   */
  private getValuesFromTableState(): ListFilterValue<TRendered>[] {
    const facets = this.getTableColumn().getFacetedUniqueValues();

    if (this.valuesFromTableState?.facets !== facets) {
      this.valuesFromTableState = {
        facets,
        values: unique(
    ...
    return this.valuesFromTableState.values;
  }
  ...
  public deleteUnknownFilterValues(): void {
    /* Values the data offers are by definition known. */
  ...
  /**
   * The values the filter offers — the ones it was given, or the ones the data
   * has.
   *
   * Follows the table, not a snapshot. A binding that keeps one filter instance
   * for the list's lifetime would otherwise hold the answer from before the
   * first batch arrived: an async loader gives the header an empty menu and
   * never fills it. The values are cached per faceted-values map, which
   * TanStack replaces when the data changes.
   */
  public get values(): ListFilterValue<TRendered>[] {
    return this._values ?? this.getValuesFromTableState();
  }
