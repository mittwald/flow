...
- **`Closes #…` works on both lines.** GitHub resolves a closing keyword only
  ...
  Most were closed by hand within a day; #3189 and #3059 sat open for weeks
  behind work that had already shipped.
  `.github/workflows/close-linked-issues.yml` supplies the missing half: on a
  merge into `next` it reads the PR body **and the PR's own commits** — `next`
  takes true merges, so both of GitHub's sources apply — and closes what the
  same PR into `main` would have closed, commenting with the PR that did it. It
  is deliberately narrower than GitHub in three places, each resolving to "leave
  it open": another repository's issue, a number that turns out to be a pull
  request, and a keyword inside a code fence. A `sync/*` head is skipped, since
  it replays `main`'s commits whose issues GitHub already closed.
